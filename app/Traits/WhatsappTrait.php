...
trait WhatsappTrait
{
    ...
    public function sendWhatsappMessage($data)
    {
        try{
            ...
            Log::info('WhatsappTrait | sendWhatsappMessage', ['Whatsapp payload' => json_encode($jsonVal)]);
    
            $res = Http::withHeaders(['Authentication' => $endpoint->key])->post($endpoint->url, $jsonVal);
            $responseBody = $res->json();
    
            Log::info('WhatsappTrait | sendWhatsappMessage', ['Sent whatsapp response' => $responseBody]);
            return $responseBody;
        } catch (\Exception $e){
